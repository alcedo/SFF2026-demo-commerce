...
- **Quantity.** Choose `+` once. The total panel shows `50 USDC` and the buy button reads `Buy now · 50 USDC`. Choose `−` once. The total returns to `25 USDC`.
- **Create order.** Choose `Buy now · 25 USDC`. The button may read `Creating order...`. The next heading is `Complete your purchase`. The URL is `/checkout/<orderId>` where `<orderId>` matches `amazon.1.<digits>.<hex>`.
...
- **Receipt.** Success heading is `Payment successful`. Status text includes `Confirmed (3/3)`. Transaction Hash is a truncated `0x…` value. Amount is `25.00 USDC`. Network is `Sepolia`.
...
- `npm run verify:purchase` buys qty 3 over HTTP (75 USDC). That is not this recipe and it does not prove the buttons.
